...
function getDegreeHoldersCount()
{
    $query = "SELECT COUNT(*) AS degree_holders
        FROM educational_background
        WHERE education_lvl_id = 5
        AND school_name != 'N/A'";
    return (select_info_multiple_key($query)[0]['degree_holders']);
}

function getRecentEmployee()
{
    $currentDate = new DateTime();
    $query = "SELECT e.emp_id, e.first_name, e.last_name, v.date_accomplished
            FROM employee e
            JOIN validation v ON e.emp_id = v.emp_id
            WHERE v.date_accomplished = '" . $currentDate->format('Y-m-d') . "'";

    return select_info_multiple_key($query);
}

function getCsEligibleCount()
{
    $query = "SELECT COUNT(emp_id) as cs_count
                FROM civil_service_eligibility 
                WHERE examination_name IS NOT NULL 
                AND examination_name != 'N/A' 
                AND examination_name != ''";
    return select_info_multiple_key($query)[0]['cs_count'];
}
...
function generateRecentEntries($employee)
{
    return "<tr class='text-center cursor-pointer'>
                <td class='w-10 px-4 py-1 text-sm'>{$employee['emp_id']}</td>
                <td class='px-4 py-2 text-sm font-bold text-slate-700'>{$employee['first_name']} {$employee['last_name']}</td>
                <td class='px-4 py-2 text-sm text-slate-700'>{$employee['date_accomplished']}</td>
            </tr>";
}

function generateRowMessage($colspan, $message)
{
    return "<tr>
                <td class='font-medium text-center px-4 py-2' colspan=$colspan>$message</td>
            </tr>";
}
